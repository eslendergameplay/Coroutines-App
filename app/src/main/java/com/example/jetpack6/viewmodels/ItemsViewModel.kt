...
import com.example.jetpack6.models.ItemsModel
...
class ItemsViewModel : ViewModel () {
    var itemList = mutableStateListOf(ItemsModel())
        private set
    /*private val _lista :MutableStateFlow<List<ItemsModel>> = MutableStateFlow(
        emptyList())
    val lista = _lista

     */
    private val _lista :MutableStateFlow<MutableList<ItemsModel>> = MutableStateFlow(
        mutableStateListOf())
    val lista = _lista
    ...
    private suspend fun llamarApi(){
        var lista = mutableListOf(ItemsModel())
        val result = withContext(Dispatchers.IO){
            ...
            // index .. 50
            for (index in index until 51){
                //itemList.add(ItemsModel(index,"Elemento $index"))
                lista.add(ItemsModel(index,"Elemento $index"))
                _lista.value = lista
            }
        }
